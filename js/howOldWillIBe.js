...
function  calculateAge(birthYear, ageYear) {
    let ageCalc = ageYear- birthYear;
    
    if(ageCalc === 0){
      return 'You were born this very year!';
    }else if(ageCalc < -1){
      return `You will be born in ${Math.abs(ageCalc)} years.`
    } else if(ageCalc === -1) {
      return `You will be born in 1 year.`
    }else if(ageCalc === 1) {
      return 'You are 1 year old.'
    }else {
      return `You are ${ageYear-birthYear} years old.`
    }
  }